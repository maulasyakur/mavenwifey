...
import { imageUploadHandler } from "./blog";
...
export const handleImageUpload = async (
  ...
  // If the caller already aborted before starting, throw immediately
  if (abortSignal?.aborted) {
    throw new Error("Upload cancelled");
  }

  // Kick off the actual upload (existing handler). We'll report progress
  // via `onProgress` while the underlying upload runs.
  const uploadPromise = imageUploadHandler(file);

  // If no progress callback provided, just await the upload and respect abort.
  if (!onProgress) {
    if (abortSignal) {
      return await Promise.race([
        uploadPromise,
        new Promise<string>((_, reject) => {
          const onAbort = () => reject(new Error("Upload cancelled"));
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    // Start by reporting 0%
    try {
      onProgress({ progress });
    } catch {
      // ignore errors thrown by progress handler
    }

    const tick = () => {
      if (abortSignal?.aborted) {
        clearInterval(timer);
        if (!finished) reject(new Error("Upload cancelled"));
        return;
      }

      // Increment progress slowly while upload is in-flight, but cap at 90%.
      if (progress < 90) {
        // random-ish step to look natural
        const step = Math.min(
          90 - progress,
